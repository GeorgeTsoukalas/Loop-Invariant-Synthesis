...
# the beta = 2.5 in the below implementations is an artifact of when I set this as fixed instead of learnable like in the cln2inv paper
class AffineFunc(LibraryFunction):
    def __init__(self, input_size, output_size, num_units, sem, *func_syms, function1 = None, function2 = None, beta = 2.5):
        ...
        self.beta = torch.nn.Parameter(torch.tensor([0.5], requires_grad = True)) # learnable parameters for the sigmoid
        self.eps = torch.nn.Parameter(torch.tensor([0.5], requires_grad=True)) # same as above, both present in cln2inv implementation
    ...
    def init_params(self):
        ...
        torch.nn.init.uniform_(self.linear_layer.weight, -1.0, 1.0) # uniform distribution between -1, 1 instead of xavier bounds which I believe are in the range [-k, k] where k = 1/sqrt(2*# vars)
    ...
    def execute_on_batch(self, batch, batch_lens=None):
        ...
        with torch.no_grad():
            self.beta.clamp_(min = 0.1) # following cln2inv implementation
...
class EqualityFunc(LibraryFunction):
    def __init__(self, input_size, output_size, num_units, sem, *func_syms, function1 = None, function2 = None, beta = 2.5):
        ...
        self.beta = torch.nn.Parameter(torch.tensor([0.5], requires_grad = True)) # learnable parameters as witnessed in cln2inv paper
    ...
    def execute_on_batch(self, batch, batch_lens=None):
        ...
        return 4*s*(torch_ones - s)

# For definitions of the t-norm, t-conorms, see the cln2inv paper
...
class LogicAndFunction(LibraryFunction):
    ...
    def __init__(self, input_size, output_size, num_units, sem, *func_syms, function1=None, function2=None):
        ...
        if function2 is None:
            function2 = init_neural_function("atom", "atom", input_size, output_size, num_units)
        self.arith = sem_here # not exactly the best naming, but for convenience
        submodules = { "function1": function1, "function2": function2 }
        grammar_symbols = None
    ...
    def execute_on_batch(self, batch, batch_lens=None):
        ...
        if self.arith == "arith":
            ...
            return torch.min(predicted_function1, predicted_function2)
        elif self.arith == "luka": # for Lukasiewicz t-norm
            return torch.max(torch.tensor(0), predicted_function1 + predicted_function2 - 1)
...
class LogicOrFunction(LibraryFunction): 
    ...
    def execute_on_batch(self, batch, batch_lens=None):
        ...
        if self.arith == "arith":
            ...
            return torch.max(predicted_function1, predicted_function2)
        elif self.arith == "luka": # for t-conorm
            return torch.min(predicted_function1 + predicted_function2, torch.tensor(1))

# Note: Below here none of these functions are *required* for the loop invariant experiments, but they are available if you need them
# In the case you need LogicNotFunction/LogicXorFunction you will need to define the semantics for these (arith, minmax, Lukasiewicz)

class FoldFunction(LibraryFunction):

    def __init__(self, input_size, output_size, num_units, fold_function=None):
        #TODO: will this accumulator require a grad? (old)
