...
class NAS(ProgramLearningAlgorithm):
    ...
    def train_graph_model(self, graph, train_loader, train_config, device, lr_decay=1.0):
        ...
        for epoch in tqdm(range(num_epoches)):
            ...
            for batchidx in tqdm(range(len(trainset))):
                ...
                # Note: As implemented here, I think the bias term may be seperately regulated
                # One might also look into the definition of the l2_regularization and see if tweaks could be made, but it is not currently in use
                ...
                if self.regularization:
                    loss = lossfxn(train_predicted, true_vals) + self.rat * l1_regularization 
                else:
                    loss = lossfxn(train_predicted, true_vals)
                ...
                # debug
                if loss.cpu().item() != loss.cpu().item(): # Not sure why this would happen, but still it is included
                    print(loss.cpu.item())
                    assert False, "Failed loss cpu check during fine tune" # Added 1/19 just to check whether this occurs
                    with torch.no_grad():
                        train_predicted_index = torch.mean(torch.mean(train_predicted, dim=-1).view(len(batch_input), -1), dim=-1) #Note: this might be different than the given loss function when changed
    ...
    def forward_backward_check(self, graph, lossfxn, optim, batch_data, label_data, output_type, output_size, device, \
        ...
        # debug
        if ratio is not None: # I don't think this happens
            loss = loss * ratio
        ...
        if early_cut: # don't think this is happening, but not sure
            print("LOSS CPU ITEM IS ", loss.cpu().item())
            assert False, "Loss cpu item fails check during architecture search" # this used to happen with the log loss until I added a positive value to avoid blowup
    ...
    def eval_graph(self, graph, test_set, eval_fun, num_labels, device):
        ...
        with torch.no_grad():
            ...
            if test_predicted != []:
                ...
                metric = 100. # TODO: this is set to something high in some weird cases where test_predicted is empty, so they are not prioritized in the heapqueue
